...
__global__ void matrixMulKernelOptimizedNoShared(float *A, float *B, float *C, int width) {
...
int main() {
    ...
    // 启动优化后的 CUDA 核函数
    matrixMulKernelOptimizedNoShared<<<numBlocks, threadsPerBlock>>>(d_A, d_B, d_C, N);
